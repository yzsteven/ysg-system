		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#roletable').bootstrapTable({
					url : '${contextPath}/shop/queryBannerListByCID', //请求后台的URL（*）
					method : 'POST', //请求方式（*）
					...
					columns : [{
                        ...
                        formatter:function(value,row,index){
                            var s = "<a><img style=\"width:70px;height:30px;\"  src=\""+row.banner+"\" /></a>";
                            return s;
                        },
                    }, {
						field : 'Button',
		...
		function operateFormatter(value, row, index) {
		      ...
		      window.operateEvents = {
		    	      'click #edit': function (e, value, row, index) {
                          parent.addMenuTab("${contextPath}/shop/toEditBanner?id="+row.id,0,"编辑轮播图");
		    	      },
		    	      'click #del': function (e, value, row, index) {
		    	        	$.ajax({
		    	        		url:"${contextPath}/shop/deleBanner",
		    	        		data:{
		    	        			id:row.id
		    	        		},
		    	        		type:"POST",
		    	        		success:function(data){
		    	        			if(data.retValue != 'success'){
		    	        				...
		    	        				alert("删除成功!");
		    	        				$("#roletable").bootstrapTable('refresh',{url:"${contextPath}/shop/queryBannerListByCID"});
		    	        			}
